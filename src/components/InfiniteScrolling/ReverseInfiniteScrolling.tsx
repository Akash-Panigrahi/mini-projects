...
function ReverseInfiniteScrolling() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const pScrollTop = container.scrollTop;
    ...
    try {
      ...
      if (response.ok) {
        ...
        setData((prev) => [...moreData, ...prev]);
        setPage((prev) => prev + 1);

        requestAnimationFrame(() => {
          const cScrollHeight = container.scrollHeight;

          container.scrollTop = cScrollHeight - pScrollHeight + pScrollTop;
        });
      }
    } catch (err) {
      console.log(err);
...
export default ReverseInfiniteScrolling;
